...
model = IncompressibleModel(        architecture = CPU(),
                                        ...
                                        buoyancy = SeawaterBuoyancy(gravitational_acceleration = g_Earth,
                                                    equation_of_state = NonLinearEquationOfState(FT),
...
η(z) = -1.4 + 0.2*interval(z,-25,-15)*exp(-(z+20)^2/4) +
    + 1.5*interval(z,-50,-40)*exp(-(z+50)^2) +
    + 1.5*interval(z,-96,-50.01)*exp(-(z+50)^2/100) +
...
salinity(z) = 29 + 4*heaviside(-z-50)*tanh(-(z+50)/25) +
...
anim = @animate for (i, iter) in enumerate(iterations)
    ...
    l = @layout [a b c; d e]
#    plot(wxy_plot, wxz_plot, uxz_plot, Tz_plot, Sz_plot, layout=l, size=(1000, 1000),
    plot(wxy_plot, wxz_plot, uxz_plot, Tz_plot, Sz_plot, layout=l,
         title = ["w(x, y, z=-8, t) (m/s)" "w(x, y=0, z, t) (m/s)" "u(x, y = 0, z, t) (m/s)" "T(x=0, y = 0, z, t) (m/s)" "S(x=0, y = 0, z, t) (m/s)"])

    iter == iterations[end] && close(file)
end

mp4(anim, "shear_turbulence.mp4", fps = 5) # hide
